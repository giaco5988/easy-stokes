...
function [U,V,X,Y] = velocity_field_1drop_channel(a,b,solution,xx1,xx2,yy,resx,resy,ite,PARAM)
  ...
  Xsing = [(a(1:n+m+j)+a(2:n+m+j+1))/2 a(n+m+j+2:end)];
  Ysing = [(b(1:n+m+j)+b(2:n+m+j+1))/2 b(n+m+j+2:end)];
  ...
  %BC stresses drop
  [df_x,df_y] = df_surf_tens_spline_symm(a(n+m+j+2:end),b(n+m+j+2:end),gamma);
  %BC poiseuille inlet
  %stresses boundary conditions
  Upoiseuille = 2*PARAM.Q/pi/PARAM.R^2*(1-(Ysing(n+m+1:n+m+j)/PARAM.R).^2);

      
  %velocities
  ux = [solution(1:2:2*n-1); zeros(m,1); Upoiseuille'; solution(2*(n+m+j)+1:2:end-1)];
  uy = [zeros(m+n+j,1); solution(2*(n+m+j)+2:2:end)];
    
  %solution
  fx = [PARAM.p_out*ones(n,1); solution(2*n+1:2:2*(n+m)-1,1); solution(2*(n+m)+1:2:2*(n+m+j)); df_x'];
  fy = [solution(2:2:2*(n+m+j)); df_y'];
...
%   A22 = [A22wall A22int];
  ...
  plot(xxx,yyy,'k')
  plot(xxx,-yyy,'k')
  plot([max(a) min(a)],PARAM.R*[1 1],'k')
  plot([max(a) min(a)],-PARAM.R*[1 1],'k')
