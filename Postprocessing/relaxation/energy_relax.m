...
%load data
load('~/Documents/MATLAB/droplet_simulations/results/relaxation/Relaxation_q=200_visc=1_Dt=0.01_loop=3000_DELTA=1.5_Ca=1_RK2.mat')
...
%     magn(i) = sum((IntY(1:end-1)+IntY(2:end)).*dx)/2;
    ...
    [df_x,df_y] = df_surf_tens_spline_symm(a,b,1);  %BC stresses
    int = df_x.*ux + df_y.*uy;
...
time = deltaT:deltaT:deltaT*stop;

figure
semilogy(time,Ps)
hold on
semilogy([time(1:step:end) time(end)],-[Pdiss(1:step:end); Pdiss(end)],'o')
hold off
grid on
xlabel('t')
ylabel('Power')
