...
PARAM.fluxBC{1} = 0;
lHere = linspace(0,1,PARAM.n(2))';
PARAM.fluxBC{2} = -lHere;
PARAM.fluxBC{3} = -1;
lHere = linspace(0,1,PARAM.n(4))';
PARAM.fluxBC{4} = lHere-1;
...
%compute slip
[vSlip1,l1] = computeSlipVelPhoretic(x{1},y{1},yLaplace(1:sum(PARAM.n(1))),PARAM,1,D1{1});
[vSlip2,l2] = computeSlipVelPhoretic(x{2},y{2},yLaplace(sum(PARAM.n(1))+1:sum(PARAM.n(1:2))),PARAM,2,D1{2});
[vSlip3,l3] = computeSlipVelPhoretic(x{3},y{3},yLaplace(sum(PARAM.n(1:2))+1:sum(PARAM.n(1:3))),PARAM,3,D1{3});
[vSlip4,l4] = computeSlipVelPhoretic(x{4},y{4},yLaplace(sum(PARAM.n(1:3))+1:sum(PARAM.n(1:4))),PARAM,4,D1{4});

%plot concentration
figure
plot([l1; l1(end)+l2; l1(end)+l2(end)+l3; l1(end)+l2(end)+l3(end)+l4],yLaplace,'x-')
hold on
plot([l1; l1(end)+l2; l1(end)+l2(end)+l3; l1(end)+l2(end)+l3(end)+l4],[vSlip1; vSlip2; vSlip3; vSlip4],'x-')
xlabel('l')
ylabel('c,\nabla c')
grid on
...
%function profile for BCs
PARAM.velBC{1} = vSlip1*(sum(PARAM.fluxBC{1})~=0);
PARAM.velBC{2} = vSlip2*(sum(PARAM.fluxBC{2})~=0);
PARAM.velBC{3} = vSlip3*(sum(PARAM.fluxBC{3})~=0);
PARAM.velBC{4} = vSlip4*(sum(PARAM.fluxBC{4})~=0);
